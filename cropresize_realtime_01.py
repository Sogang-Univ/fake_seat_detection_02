...
SHOW_PREVIEW = True
STREAM_PORT = 8090
SHOW_LOCAL_WINDOW = True   # ssh -X(X11 forwarding) 접속 시에만 True로 사용
...
class SaveWorker(threading.Thread):
    ...
    def __init__(self, writer, npy_data, max_frames, live_stream=None, show_window=True):
        super().__init__(daemon=True)
        self.writer = writer
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                if self.show_window: 
                    cv2.imshow("ROI Crop+Resize (live)", resized)
                    cv2.waitKey(1)
...
# FOURCC 를 먼저 설정해야 한다. YUYV 로 negotiate 되면
# 640x480 에서도 USB 대역폭 때문에 5~10 FPS 로 떨어지는 경우가 많다.
cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*"MJPG"))
cap.set(cv2.CAP_PROP_FRAME_WIDTH,  INPUT_SHAPE[1])
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, INPUT_SHAPE[0])
...
try:
    ...
    while True:
        ...
        if processed % STATS_EVERY_N_FRAMES == 0:
            print("%5d frame | PRE %5.2f ms | PL %6.3f ms | SYS %5.1f FPS | drop %d"
